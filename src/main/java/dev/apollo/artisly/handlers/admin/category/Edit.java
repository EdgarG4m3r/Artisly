...
public class Edit implements APIHandler {
    ...
    public void handle(Context context) {
        ...
        try
        {
            ...
            if (result)
            {
                StandarizedResponses.success(context, "SUCCESS", "Berhasil mengupdate kategori!");
            }
            else
            {
                StandarizedResponses.generalFailure(context, 400, "FAILED", "Gagal mengupdate kategori!");
            }

        } catch (SQLException e) {
            StandarizedResponses.generalFailure(context, 500, "SQL_EXCEPTION", "Gagal saat mengupdate kategori!");
